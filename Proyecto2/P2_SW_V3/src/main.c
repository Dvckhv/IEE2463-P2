...
int main()
{
	...
	while(1){
		...
		if (read_acx() >= 469 && read_acx() <= 581){
			posicion = read_acx() - 468;
			xil_printf("Posición: %d\r\n", posicion);
			GUI_Mover_tanque(posicion, prev_posicion);
			prev_posicion = posicion;
		}
		else if (read_acx() < 469){
			xil_printf("Posición: 1\r\n");
		}
		else if (read_acx() > 581){
			xil_printf("Posición: 113\r\n");
		}
